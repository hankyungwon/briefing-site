...
(async () => {
  ...
  await page.click("#memo-last"); await page.type("#memo-last", "크기 시험");
  await H.selAll(page, "memo-last");
  await page.click('#memo-toolbar [data-cmd="size-up"]'); await page.waitForTimeout(100);
  const size = await page.evaluate(() => ({ preset: !!document.querySelector("#memo-toolbar .rt-size"), num: document.querySelector("#memo-toolbar .rt-sizenum").value, html: document.getElementById("memo-last").innerHTML }));
  ...
  const red = await page.evaluate(() => document.getElementById("memo-last").innerHTML);
  ...
  await page.click("#memo-this"); await page.type("#memo-this", "강조 문장");
  await H.selAll(page, "memo-this");
  ...
  c.ok(/background-color/i.test(await page.evaluate(() => document.getElementById("memo-this").innerHTML)), "형광펜 적용");

  // 6) 다단계 번호 (1. → 가. → 1) → 가) → (1))
  const markers = await page.evaluate(() => {
    const ed = document.getElementById("memo-last");
  ...
  c.ok(await page.evaluate(() => getComputedStyle(document.querySelector("#memo-last ol")).listStyleType) === "kr-circle", "번호 스타일 ① 변경");

  // 8) 줄 간격
  await page.evaluate(() => { document.getElementById("memo-this").innerHTML = "<p>줄간격 시험 문단</p>"; });
  await H.selAll(page, "memo-this");
  await page.selectOption("#memo-toolbar .rt-linespace", "2.0"); await page.waitForTimeout(100);
  c.ok(/line-height:\s*2/i.test(await page.evaluate(() => document.getElementById("memo-this").innerHTML)), "줄간격 2.0 적용");

  // 9) 문자표 — 자주쓰기 첫 탭·삽입·최근 사용
  await page.evaluate(() => { const ed = document.getElementById("memo-last"); ed.innerHTML = "<p>기호 </p>"; ed.focus(); const r = document.createRange(); r.selectNodeContents(ed); r.collapse(false); const s = getSelection(); s.removeAllRanges(); s.addRange(r); ed.dispatchEvent(new Event("mouseup")); });
  ...
  const ins = await page.evaluate(() => ({ body: document.getElementById("memo-last").innerText, recent: [...document.querySelectorAll("#char-recent button")].map(b => b.dataset.ch) }));
  ...
  await page.evaluate(() => {
    ...
    const ed = document.getElementById("memo-last"); ed.innerHTML = '<p>사진</p><img src="' + cv.toDataURL("image/png") + '" style="max-width:100%;"><p><br></p>'; ed.dispatchEvent(new Event("input"));
  });
  await page.click("#memo-last img"); await page.waitForTimeout(200);
  c.ok(await page.evaluate(() => document.querySelectorAll("#pk-imgbox .ib-h").length) === 8, "그림 선택 시 8방향 핸들");
  const nat = await page.evaluate(() => { const i = document.querySelector("#memo-last img"); return { w: i.offsetWidth, h: i.offsetHeight }; });
  ...
  const aE = await page.evaluate(() => { const i = document.querySelector("#memo-last img"); return { w: i.offsetWidth, h: i.offsetHeight }; });
  ...
  const aSE = await page.evaluate(() => { const i = document.querySelector("#memo-last img"); return { w: i.offsetWidth, h: i.offsetHeight }; });
  ...
  await page.evaluate(() => { const ed = document.getElementById("memo-this"); ed.innerHTML = "<p><br></p>"; ed.focus(); const s = getSelection(); s.selectAllChildren(ed); s.collapseToEnd(); });
  await page.click('#memo-toolbar [data-cmd="table"]'); await page.waitForTimeout(200);
  const putCaret = () => page.evaluate(() => { const td = document.querySelector("#memo-this table td"); const r = document.createRange(); r.selectNodeContents(td); r.collapse(true); const s = getSelection(); s.removeAllRanges(); s.addRange(r); });
  ...
  const t1 = await page.evaluate(() => { const tb = document.querySelector("#memo-this table"); return tb.querySelectorAll("tr").length + "x" + tb.querySelector("tr").children.length; });
  ...
  const t2 = await page.evaluate(() => { const tb = document.querySelector("#memo-this table"); return tb.querySelectorAll("tr").length + "x" + tb.querySelector("tr").children.length; });
